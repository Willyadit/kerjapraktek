...

    {
        ...
        $data = [
            'barang1' => DB::table('barangs')
                        ->join('kategori','barangs.kategori_id','=','kategori.id_kategori')
                        ->select('barangs.*','kategori.*')->get(),
            'hitung' => count($barang2),
            'kategori' => DB::table('kategori')->get(),
            'barang' =>DB::table('barangs')->sum('jumlah'),
        ];
        
        return view('barang.view')->with($data);        
        // return view('barang.view', compact('barang2','barang','hitung','kategori'));
    }
